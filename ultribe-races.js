...
.nav-btn{
  ...
  padding:8px 22px;font-size:13px;font-weight:700;text-transform:uppercase;
...
.hero-eyebrow{font-size:11px;text-transform:uppercase;letter-spacing:2.5px;color:var(--o);font-weight:700;margin-bottom:6px}
.hero-title{font-size:clamp(26px,6vw,42px);font-weight:800;letter-spacing:-.5px;line-height:1.05}
...
.hero-pill{
  ...
  color:var(--o);font-size:11px;font-weight:700;
...
.cat-tab .td{font-size:17px;font-weight:800;color:var(--muted);line-height:1;transition:color .18s}
.cat-tab .tn{font-size:10px;text-transform:uppercase;letter-spacing:1px;font-weight:600;color:var(--muted);margin-top:3px;white-space:nowrap;transition:color .18s}
...
.stat-val{font-size:16px;font-weight:800;color:var(--ink);margin-top:3px}
...
.tile.wide h4{color:#fff;font-size:15px}
.tile.wide p{color:rgba(255,255,255,.72);font-size:13px}
.tile-icon{
  width:36px;height:36px;border-radius:10px;
  background:var(--o10);border:1.5px solid var(--o20);
  display:flex;align-items:center;justify-content:center;
  margin-bottom:12px;font-size:17px;line-height:1;
}
...
.tile-badge{
  ...
  letter-spacing:.8px;padding:2px 10px;border-radius:30px;margin-bottom:10px;
}
.tile h4{font-size:14px;font-weight:700;color:var(--ink);margin-bottom:5px;line-height:1.3}
.tile p{font-size:12.5px;color:var(--muted);line-height:1.55}
...
@media(max-width:600px){
  ...
  .cat-tab .td{font-size:15px}
  .info-tab{font-size:11px;padding:6px 12px}
...
<div>
  ...
  <div id="panels">
    ...
    const getPanel = (k, t) => root.getElementById(`p-${k}-${t}`);

    const setStats = (k) => {
      const s = STATS[k];
      Sstart.innerHTML  = `<span>${s.start}</span>`;
      Scutoff.textContent = s.cutoff;
      Selev.textContent   = s.elev;
      Sitra.innerHTML   = `<span>${s.itra}</span>`;
    ...
    const switchInfo = (t) => {
      if (t === curT) return;
      getPanel(curK, curT)?.classList.remove('on');
      root.querySelectorAll('.info-tab').forEach(tb =>
        tb.classList.toggle('on', +tb.dataset.t === t));
      requestAnimationFrame(() => {
